...
export class RegisterPage implements OnInit {
  ...
  447700900555
  updateMask(event: any) {
    const inputValue = event.target.value.replace(/\D/g, ''); // Remove não dígitos
    this.phoneNumber = inputValue;
  
    if (inputValue.length == 11 && this.igreja == 'Aprisco Sede' || inputValue.length <= 11 && this.igreja == 'Aprisco SSA') {
      this.phoneMask = '(00) 00000 0000'; // Brasil
    } 
  }
...
// Funciton to ensure passwords match
 ...
 // Function to check if e-mail is taken
  checkPhone() {
    ...
    this.authService.checkPhoneExist(newValor).subscribe((data: any) => {
      console.log(data.response.numberExists)
    })
 ...
 internaciona(){
  this._route.navigate(['/international-register'])
 }
  ngOnInit() {
  }
  doRefresh(event) {
